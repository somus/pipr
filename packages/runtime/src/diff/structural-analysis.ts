...
async function loadBaseOutlines(options: {
  workspace: string;
  root: string;
  ref: string;
  paths: string[];
  ...
  for (const filePath of options.paths) {
    const result = Bun.spawnSync(["git", "show", `${options.ref}:${filePath}`], {
      cwd: options.workspace,
      env: options.env ?? process.env,
      maxBuffer: outlineStdoutLimitBytes,
      stdin: "ignore",
      stdout: "pipe",
      stderr: "pipe",
    });
    if (result.exitCode !== 0) {
      throw new StructuralAnalysisError("base-content-unavailable");
    }
