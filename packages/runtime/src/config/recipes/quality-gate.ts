...
export default definePipr((pipr) => {
  ...
  const reviewer = pipr.agent({
    ...
    instructions: \`
      ...
      verify. Include suggestedFix only for exact replacements that change the
      selected lines; omit it for no-op, trailing-blank-line-only, or
      whitespace-only changes. The blocker body must describe the issue that
      suggestedFix directly fixes. Omit suggestedFix for secrets, credentials,
      API keys, tokens, or config wiring unless the replacement uses an existing
      secret, environment variable, or config key already present in the
      surrounding code. If no blocking issue exists, return an empty blockers
      array.
    \`,
