...
export default definePipr((pipr) => {
  ...
  const task = pipr.task({
    ...
    async run(ctx) {
      ...
      const calls = 1 + unitResults.length + (concurrency ? 1 : 0);

      await ctx.comment({
        main: \`Deep review completed with \${calls} reviewer call\${calls === 1 ? "" : "s"}.\`,
...
function hasConcurrencySignals(manifest: DiffManifest): boolean {
  const changedText = manifest.files
    ...
    .join("\\n");
  return /\\b(?:async|await|promise|goroutine|mutex|lock|thread|queue|cache|retryCount|concurrent|atomic|shutdown)\\b/i.test(
    changedText,
  );
}
