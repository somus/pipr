  ...
  const task = pipr.task({
    ...
    async run(ctx) {
      ...
      const inlineFindings: ReviewFinding[] = result.findings.map((finding) => ({
        body: \`**\${formatPolicy(finding.policy)}:** \${finding.title}. \${finding.body}\`,
        path: finding.path,
...
function policyTable(checks: PolicyCheck[]): string {
  ...
  return [
    ...
    ...checks.map(
      (check) =>
        \`| \${formatPolicy(check.policy)} | \${formatStatus(
          check.status,
        )} | \${escapeTableCell(check.evidence)} |\`,
    ),
  ].join("\\n");
}
...
function findingsTable(findings: HygieneFinding[]): string {
  ...
  return [
    ...
    ...findings.map(
      (finding) =>
        \`| \${formatPolicy(finding.policy)} | \${escapeTableCell(finding.title)} |\`,
    ),
  ].join("\\n");
}

function formatPolicy(policy: string): string {
  return policy.replaceAll("-", " ").replace(/^./, (char) => char.toUpperCase());
}

function formatStatus(status: string): string {
  return status.replaceAll("-", " ").replace(/^./, (char) => char.toUpperCase());
}
