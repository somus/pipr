...
describe("initOfficialMinimalProject: project scaffolding and safety", () => {
  ...
  it("creates the official minimal .pipr tree and validates it", async () => {
    ...
    expect(workflow).toContain("uses: somus/pipr@v0.7.0"); // x-release-please-version
  ...
  it("creates an opt-in GitLab merge request pipeline", async () => {
    ...
    expect(pipeline).toContain("ghcr.io/somus/pipr:v0.7.0"); // x-release-please-version
  ...
  it("creates Gitea, Forgejo, and Codeberg Actions scaffolding", async () => {
    for (const adapter of ["gitea", "forgejo", "codeberg"] as const) {
      ...
      expect(workflow).toContain("ghcr.io/somus/pipr:v0.7.0"); // x-release-please-version
