...
describe("comments", () => {
  ...
  it("renders collapsed review stats before the attribution", () => {
    const plan = buildPublicationPlan({
      ...
      metadata: {
        ...
        stats: {
          ...
          cacheReadTokens: 345_678,
          cacheWriteTokens: 12_345,
          cacheUsageStatus: "complete" as const,
          diffContextCoverage: {
            files: { total: 7, covered: 6 },
            ranges: { total: 24, covered: 21 },
          },
        },
      },
    });
    ...
    expect(plan.mainComment).toContain(
      [
        ...
        "| Output tokens | 8,900 |",
        "| Cache read tokens | 345,678 |",
        "| Cache write tokens | 12,345 |",
        "| Current-run files with full context | 6 / 7 |",
        "| Current-run ranges with full context | 21 / 24 |",
        "| Cost (USD) | $0.0035 |",
        "",
        "</details>",
        "<!-- pipr:stats:end -->",
  ...
  it("adds the hosted workflow URL to completion details", () => {
    const plan = buildPublicationPlan({
      event,
      main: "Summary body.",
      inlineItems: [],
  ...
  it("independently hides generated main comment presentation", () => {
    ...
    expect(withoutStats.mainComment).not.toContain("<summary>Review completed");
