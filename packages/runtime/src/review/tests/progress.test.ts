...
import { reviewProgressEndMarker } from "../comment-branding.js";
import {
...
describe("review progress", () => {
  it("renders a minimal accessible first-run comment with a decorative image", () => {
    ...
    expect(body).toContain("✅ Preparing workspace");
    expect(body).toContain("○ Validating review");
  ...
  it("distinguishes active tasks, reviewers, and shards beneath the stable stage", () => {
    ...
    expect(body).toContain("⏳ <strong>Running: Running review tasks</strong>");
    expect(body).toContain("<strong>Task:</strong> <code>review &lt;core&gt;</code>");
    expect(body).toContain(
      "↳ <strong>Reviewer:</strong> <code>security &amp; trust</code> · shard 2 of 4",
    );
    expect(body).toContain(
      "↳ <strong>Reviewer:</strong> <code>maintainability</code> · shard 1 of 4",
    );
    expect(body).toContain("<strong>Completed:</strong> 6 review runs · 2 reviewers active");
  ...
  it("bounds concurrent reviewer rows and summarizes overflow", () => {
    ...
    expect(body.match(/<strong>Reviewer:<\/strong>/g)).toHaveLength(3);
