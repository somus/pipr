...
The follow-up Cloud fixture separated the cases. Bitbucket rendered a filename change plus a one-line edit as a renamed file with one added and one removed line, and rendered an unchanged filename move as a pure renamed file with no content delta. This proves native rename classification, while inline publication on the edited rename still depends on the normal destination and previous-path mapping.

Two attempted range comments on a three-line replacement serialized through Bitbucket's authenticated UI proxy as single-line bodies: `{ "inline": { "path": "fixture.txt", "from": 2 } }` for the deleted side and the corresponding `to` form for the added side. Direct calls to the public REST endpoint completed the proof: an added-side request returned `start_to: 2`, `to: 4`, and three added context lines, while a deleted-side request returned `start_from: 2`, `from: 4`, and three deleted context lines. Multiline capability is therefore enabled. The current UI path did not produce a submitted code suggestion, so suggestion encoding remains a mandatory authenticated REST test before that capability is enabled.

Separate scoped user tokens also proved the remaining operational contracts. A token with only `read:repository:bitbucket`, owned by the workspace administrator, returned effective `admin` for the fixture repository. Two source-commit status writes with key `pipr-review` left one stored status in the final `SUCCESSFUL` state. For partial publication, Cloud retained the first marked inline comment after rejecting the second request with `inline.to: expected int`; the retry skipped the retained marker and created only the missing marker, leaving exactly one of each.
...
A later edit retained `changeTrackingId: 1`. A pure repository rename then appeared in PR iteration changes as a delete of the original path with tracking ID 1 and an add of the destination path with tracking ID 2, rather than one rename entry. A left-side thread on the deleted path and a right-side thread on the destination path both returned HTTP 200 and rendered in the UI with `{ firstComparingIteration: 1, secondComparingIteration: 3 }`. The adapter must index both current and original paths and handle this split representation.

The retained PR was then retargeted from `main` to a new branch. Azure created iteration 4 with the UI comparison link `iteration=4&base=3`; all four existing positioned threads remained attached to their files, including deleted-side, multiline Unicode, rename-destination, and later-iteration anchors. A subsequent commit to the target branch changed the target SHA without creating another source iteration, and the same four threads remained tracked. This proves that selecting the latest iteration matching the reviewed source head is required after retargeting, while the stale-base check must use the pull request's current target commit rather than assume every target update creates an iteration.

The Azure UI rendered a fenced `suggestion` reply as ordinary code and exposed no Apply action. PR-Agent's Azure provider uses the same ordinary thread-comment API and does not send a replacement payload, so Azure native suggestions remain disabled and Pipr preserves the proposed replacement in a visible plain code block. Suggested-change encoding remains disabled because the public thread API still has no documented or stable live payload for it.
...
The REST reference exposes `from`, `to`, `start_from`, `start_to`, and `path` but does not explain all valid combinations or server behavior for mixed/context ranges. The canonical field schema is also available in Atlassian's [Bitbucket OpenAPI document](https://developer.atlassian.com/cloud/bitbucket/swagger.json). The July 2026 Cloud fixture proved single-line anchors, added and deleted multiline ranges through direct REST calls, and native classification for rename-with-edit and pure rename. One deliberately out-of-range `to: 999` request was accepted with empty context, so Pipr must continue validating line bounds locally before publication. Treat outdated comments as a required live API spike before declaring parity.
...
For a full-parity local PAT, select `Code (Read & write)`, `Code (Status)`, `Identity (Read)`, and `Security (Manage)` and scope it to one organization. The identity lookup requires `vso.identity`. Azure DevOps exposes `vso.security_manage`, rather than a read-only Security PAT scope, even when the adapter only queries ACLs. Installations that disable commands can omit the Identity and Security scopes because they do not need effective actor-permission evaluation.
...
List iterations and choose the latest iteration whose source commit matches the reviewed head. Iteration one is created with the PR and later pushes create later iterations. Fetch `GET .../iterations/{iterationId}/changes` with `compareTo=0`, read entries from `changeEntries`, and follow nonzero `nextSkip` and `nextTop` values. Each change supplies `changeTrackingId`, current path, original path, and change type. The maximum page size is 2000. See [Pull Request Iteration Changes](https://learn.microsoft.com/en-us/rest/api/azure/devops/git/pull-request-iteration-changes/get?view=azure-devops-rest-7.1).
...
Do not copy `compareTo=0` from the iteration-changes read into the thread request. Live Azure DevOps Services testing returned HTTP 400 for `firstComparingIteration: 0`, accepted `1/1` for the initial PR, and accepted `1/2` and `1/3` after later pushes. The same tests proved right-side multiline, left-side delete, split rename anchors, and preservation of all four positioned threads after retarget iteration 4 plus a later target-branch commit. Do not infer `changeTrackingId` from local diff order.
