...
expected.set("meta.json", renderMeta());
...
function renderIndex(): string {
  const catalogRows = recipes
    .map(
      (recipe) =>
        `| [${recipe.title}](/docs/recipes/${recipe.id}) | ${recipeDescription(recipe)} |`,
    )
    .join("\n");
...
function renderRecipe(recipe: (typeof recipes)[number]): string {
  ...
  const metadataDescription = plainTextDescription(description);
...
description: "${escapeFrontmatter(metadataDescription)}"
---
...
These are the complete files written by \`${command}\`:
