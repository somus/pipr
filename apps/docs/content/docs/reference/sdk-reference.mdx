...
Pass `{ manifest }` to specialists that need changed-code context. Pipr injects the authoritative Diff Manifest section from that reserved key. Omit `manifest` for aggregators that only merge specialist results.
