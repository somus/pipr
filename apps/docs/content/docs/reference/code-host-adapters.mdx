...
The three host ids share one Gitea-compatible API family while retaining provider-specific environment variables, workflow paths, URLs, and diagnostics. The adapter maps pull requests, issue comments, review comments, collaborator permissions, and commit statuses into the common contract. It publishes single-line inline comments and verifier replies, but it does not claim native thread resolution, multiline anchors, or native suggestion blocks.
