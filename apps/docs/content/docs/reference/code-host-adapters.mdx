...
The Bitbucket Cloud adapter maps repository webhooks, pull request comments, effective workspace permissions, and source-commit statuses into the same runtime contract. It uses the public `from`, `to`, `start_from`, and `start_to` inline anchor fields and checks both pull request endpoints before writes.
