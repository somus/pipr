...
GitHub.com, GitHub Enterprise Server, GitLab.com, GitLab Self-Managed, Azure DevOps Services, Azure DevOps Server, Bitbucket Cloud, Bitbucket Data Center, Gitea, Forgejo, and Codeberg are supported.
...
The Bitbucket adapter maps Cloud and Data Center repository webhooks, pull request comments, effective permissions, and source-commit statuses into the same runtime contract. Cloud uses `from` and `to` anchors with optional multiline starts; Data Center 9.2 and later uses commit diff anchors with `multilineMarker`. Both paths check pull request endpoints before writes.

See [Bitbucket](/docs/guide/bitbucket) for Cloud and Data Center setup and permission-token requirements.
