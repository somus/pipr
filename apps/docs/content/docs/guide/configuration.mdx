...
`pipr init` scaffolds a tier-2 Bun package under `.pipr/` with `@usepipr/sdk` and `@types/bun` pinned in `.pipr/package.json`. Run `bun install` in `.pipr/` (init does this automatically) to refresh types after bumping the SDK version. For a single-file config without install, use `pipr init --minimal` and install `@usepipr/sdk` at the repo root for editor types.
