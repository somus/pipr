...
This page shows common patterns. For triggers and commands, see [Entrypoints](/docs/guide/entrypoints). For task handlers, see [Custom Tasks](/docs/guide/custom-tasks). For the full public API, see [Pipr SDK Reference](/docs/reference/sdk-reference). For generated starter configs, see [Init Recipes](/docs/recipes).
...
`pipr.review` registers a change request review task and the `@pipr review` command unless `entrypoints` disables them.
