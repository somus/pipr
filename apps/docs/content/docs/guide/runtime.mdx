...
`change.number` is provider-relative: a GitHub pull request number or GitLab merge request IID. Bitbucket Cloud and Azure DevOps Services follow the same provider-neutral contract in their stacked releases.
