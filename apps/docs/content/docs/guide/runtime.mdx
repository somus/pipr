...
`change.number` is provider-relative: a GitHub or Azure DevOps pull request number, or a GitLab merge request IID. Bitbucket Cloud follows the same provider-neutral contract in its stacked release.
