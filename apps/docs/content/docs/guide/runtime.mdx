...
Before publishing, Pipr checks the current head SHA. It publishes Inline Review Comments and thread actions first, then upserts the Main Review Comment only after inline publication succeeds. The main comment is the publication watermark: a partial inline failure leaves its prior body and state unchanged. A rerun reconciles Pipr-owned markers, skips inline comments the earlier attempt already published, and advances the main comment after the remaining work succeeds.
